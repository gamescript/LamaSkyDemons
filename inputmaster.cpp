    ...
    Object(context)
{
    ...
    keyBindingsPlayer_[2][KEY_LSHIFT] = PlayerInputAction::JUMP;
...
void InputMaster::HandleActions(const InputActions& actions)
{
    ...
    for (Player* p : MC->GetPlayers()){
        ...
        if (controlled){
            ...
            restActions[0] = playerInputActions->Contains(PlayerInputAction::JUMP);
            restActions[1] = playerInputActions->Contains(PlayerInputAction::SPIT);
